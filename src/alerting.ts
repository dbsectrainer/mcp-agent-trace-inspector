...
function computeAverageLatencyMs(db: DatabaseSync): number {
...
function computeErrorRatePct(db: DatabaseSync): number {
  const traces = listTraces(db);
  if (traces.length === 0) return 0;
...
function computeTotalCostUsd(db: DatabaseSync): number {
  const traces = listTraces(db);
  let totalTokens = 0;
...
function buildSlackBlocks(alert: AlertFired): unknown {
  return {
    blocks: [
      {
        type: "section",
...
export async function checkAndAlert(
  ...
  for (const rule of rules) {
    ...
    if (value > rule.threshold) {
      const alert: AlertFired = { rule, value, message };
      fired.push(alert);
...
export function initAlertRulesTable(db: DatabaseSync): void {
  db.exec(`
...
export function saveAlertRules(db: DatabaseSync, rules: AlertRule[]): void {
...
export function loadAlertRules(db: DatabaseSync): AlertRule[] {
  initAlertRulesTable(db);
  const rows = db
    .prepare("SELECT type, threshold FROM alert_rules")
    .all() as unknown as Array<{ type: string; threshold: number }>;
  return rows.map((r) => ({
