...
export function insertTrace(db: DatabaseSync, id: string, name: string): void {
  db.prepare(
    `INSERT INTO traces (id, name, status, started_at) VALUES (?, ?, 'running', ?)`,
  ).run(id, name, Date.now());
}

export function endTrace(db: DatabaseSync, id: string): void {
  db.prepare(
    `UPDATE traces SET status = 'completed', ended_at = ? WHERE id = ?`,
  ).run(Date.now(), id);
}

export function insertStep(
  db: DatabaseSync,
  step: Omit<StepRow, "created_at">,
): void {
  db.prepare(
    `
...
export function getTrace(db: DatabaseSync, id: string): TraceRow | undefined {
  return db.prepare(`SELECT * FROM traces WHERE id = ?`).get(id) as unknown as
    | TraceRow
    | undefined;
}

export function getSteps(db: DatabaseSync, traceId: string): StepRow[] {
  return db
    .prepare(`SELECT * FROM steps WHERE trace_id = ? ORDER BY created_at ASC`)
    .all(traceId) as unknown as StepRow[];
}

export function listTraces(db: DatabaseSync, limit?: number): TraceRow[] {
  if (limit && limit > 0) {
    return db
...
export function deleteOldTraces(
  db: DatabaseSync,
