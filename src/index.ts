...
async function main(): Promise<void> {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option("retention-days", {
      ...
      default: 0,
      description:
        "Auto-delete traces older than this many days. 0 = disabled.",
    })
    ...
    .option("http-port", {
      type: "number",
      description:
        "Start in HTTP mode on the given port instead of stdio. Example: --http-port=3000",
    })
    .help()
    .parseAsync();
  ...
  if (argv["pricing-table"]) {
    try {
      ...
      console.error(
        `[init] Loaded custom pricing table with ${Object.keys(customPricing).length} model(s)`,
      );
    } catch (err) {
  ...
  const retentionDays = argv["retention-days"] as number;
  if (retentionDays > 0) {
    const deleted = deleteOldTraces(db, retentionDays);
    if (deleted > 0) {
      console.error(
        `[retention] Deleted ${deleted} trace(s) older than ${retentionDays} day(s)`,
      );
    }
  }

  const noTokenCount = argv["no-token-count"] as boolean;
