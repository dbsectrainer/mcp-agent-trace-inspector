...
let _retentionDays: number | null = null;
// Alert channels storage
let _alertChannels: AlertChannels = {};
...
export function createServer(options: ServerOptions): Server {
  ...
  const server = new Server(
    { name: "mcp-agent-trace-inspector", version: "0.2.0" },
  ...
  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    return handleGetPrompt(
      name,
      promptArgs as Record<string, string> | undefined,
    );
  });
        ...
        inputSchema: {
          ...
          properties: {
            name: {
              type: "string",
              description:
                'Human-readable name for this trace/workflow run. Example: "Product Search Workflow 2024-01-15". Pass "auto" to auto-generate.',
            },
          },
          required: ["name"],
        ...
        inputSchema: {
          ...
          properties: {
            trace_id: {
              type: "string",
              description:
                'ID of the trace to record this step under. Example: "abc-123-def-456"',
            },
            tool_name: {
              type: "string",
              description:
                'Name of the tool or action that was executed. Example: "web_search" or "llm_call"',
            },
            input: {
              type: "object",
              description:
                'Input payload passed to the tool. Example: { "query": "weather in Paris" }',
            },
            output: {
              type: "object",
              description:
                'Output/result returned by the tool. Example: { "result": "Sunny, 22°C" }. Add "error" field or "isError": true to flag errors.',
            },
            token_count: {
              type: "number",
              description:
                "Optional number of tokens consumed in this step. Example: 150",
            },
            latency_ms: {
              type: "number",
              description:
                "Optional wall-clock latency of this step in milliseconds. Example: 450",
            },
          },
        ...
        inputSchema: {
          ...
          properties: {
            trace_id: {
              type: "string",
              description:
                'ID of the trace to complete. Example: "abc-123-def-456"',
            },
          },
          required: ["trace_id"],
          additionalProperties: false,
        ...
        inputSchema: {
          ...
          properties: {
            trace_id: {
              type: "string",
              description:
                'ID of the trace to summarize. Example: "abc-123-def-456"',
            },
            model: {
              type: "string",
              description:
                'Model name for cost estimation. Example: "claude-sonnet-4-6". Defaults to "claude-sonnet-4-6".',
            },
          },
          required: ["trace_id"],
          additionalProperties: false,
        ...
        inputSchema: {
          ...
          properties: {
            limit: {
              type: "number",
              description:
                "Maximum number of traces to return (most recent first). Example: 20. Omit for all.",
            },
          },
          additionalProperties: false,
        },
        ...
        inputSchema: {
          ...
          properties: {
            trace_id: {
              type: "string",
              description:
                'ID of the trace to export. Example: "abc-123-def-456"',
            },
          },
          required: ["trace_id"],
          additionalProperties: false,
        ...
        inputSchema: {
          ...
          properties: {
            trace_id_a: {
              type: "string",
              description:
                'ID of the first trace (baseline). Example: "abc-123"',
            },
            trace_id_b: {
              type: "string",
              description:
                'ID of the second trace (to compare against). Example: "def-456"',
            },
          },
        ...
        inputSchema: {
          ...
          properties: {
            trace_id: {
              type: "string",
              description:
                'ID of the trace to extract reasoning from. Example: "abc-123-def-456"',
            },
          },
          required: ["trace_id"],
          additionalProperties: false,
        ...
        inputSchema: {
          ...
          properties: {
            trace_id: {
              type: "string",
              description:
                "Optional trace ID to export. Omit to export all traces.",
            },
            format: {
              type: "string",
              enum: ["json"],
        ...
        inputSchema: {
          ...
          properties: {
            ...
            slack_webhook: {
              type: "string",
              description:
                "Optional Slack webhook URL for Slack block notifications.",
            },
            webhook_url: {
              type: "string",
              description:
                "Optional generic webhook URL for JSON POST notifications.",
            },
          },
          required: ["rules"],
          additionalProperties: false,
        ...
        inputSchema: {
          ...
          properties: {
            retention_days: {
              type: "number",
              description:
                "Number of days to retain traces before archiving. Must be > 0.",
            },
          },
        ...
        inputSchema: {
          ...
          properties: {
            from_date: {
              type: "string",
              description:
                "Optional ISO date string to filter entries from (inclusive).",
            },
            to_date: {
              type: "string",
              description:
                "Optional ISO date string to filter entries to (inclusive).",
            },
            format: {
              type: "string",
              enum: ["json", "csv"],
      ...
      switch (name) {
        case "trace_start": {
          const result = handleTraceStart(db, {
            name: safeArgs.name as string,
          });
          const startData = JSON.parse(result.content[0].text) as {
            trace_id: string;
            name: string;
          };
        ...
        case "trace_step": {
          const result = handleTraceStep(
            db,
            {
              trace_id: safeArgs.trace_id as string,
              tool_name: safeArgs.tool_name as string,
              input: safeArgs.input as object,
              output: safeArgs.output as object,
              token_count:
                safeArgs.token_count !== undefined
                  ? (safeArgs.token_count as number)
                  : undefined,
              latency_ms:
                safeArgs.latency_ms !== undefined
                  ? (safeArgs.latency_ms as number)
                  : undefined,
            },
            noTokenCount,
          );
          // Emit debug notification for each recorded step
          const stepData = JSON.parse(result.content[0].text) as {
            step_id: string;
            tool_name: string;
          };
          await server.notification({
        ...
        case "trace_end": {
          ...
          const endData = JSON.parse(result.content[0].text) as {
            trace_id: string;
            duration_ms: number;
          };
          await server.notification({
        ...
        case "get_trace_summary":
          return handleGetTraceSummary(db, {
            trace_id: safeArgs.trace_id as string,
            model:
              safeArgs.model !== undefined
                ? (safeArgs.model as string)
                : undefined,
          });
          ...
          return handleListTraces(db, {
            limit:
              safeArgs.limit !== undefined
                ? (safeArgs.limit as number)
                : undefined,
          });

        case "export_dashboard":
          return handleExportDashboard(
            db,
            {
              trace_id: safeArgs.trace_id as string,
            },
        ...
        case "compare_traces": {
          const result = handleCompareTraces(db, {
            trace_id_a: safeArgs.trace_id_a as string,
            trace_id_b: safeArgs.trace_id_b as string,
          });
          await server.notification({
            method: "notifications/message",
            params: {
              ...
              data: `Traces compared: a=${safeArgs.trace_id_a as string} b=${safeArgs.trace_id_b as string}`,
            },
          });
          return result;
        }
        ...
        case "extract_reasoning_chain": {
          const traceId = safeArgs.trace_id as string;
          ...
          if (!summary) {
            throw new McpError(
              ErrorCode.InvalidParams,
              `Unknown trace_id: ${traceId}`,
            );
          }
          const REASONING_PATTERNS =
            /reason|think|plan|reflect|analyz|consider/i;
          ...
          const json = {
            ...
            steps: reasoningSteps.map((s, i) => ({
              ...
              input: (() => {
                try {
                  return JSON.parse(s.input_json) as unknown;
                } catch {
                  return s.input_json;
                }
              })(),
              output: (() => {
                try {
                  return JSON.parse(s.output_json) as unknown;
                } catch {
        ...
        case "export_otel": {
          const traceId =
            safeArgs.trace_id !== undefined
              ? (safeArgs.trace_id as string)
              : undefined;
          let result;
          if (traceId) {
        ...
        case "configure_alerts": {
          const rules = safeArgs.rules as AlertRule[];
          const slackWebhook =
            safeArgs.slack_webhook !== undefined
              ? (safeArgs.slack_webhook as string)
              : undefined;
          const webhookUrl =
            safeArgs.webhook_url !== undefined
              ? (safeArgs.webhook_url as string)
              : undefined;
        ...
        case "set_retention_policy": {
          const retentionDays = safeArgs.retention_days as number;
        ...
        case "apply_retention": {
          if (_retentionDays === null) {
            throw new McpError(
              ErrorCode.InvalidParams,
              "No retention policy set. Call set_retention_policy first.",
            );
          }
        ...
        case "export_compliance_log": {
          const fromDate =
            safeArgs.from_date !== undefined
              ? (safeArgs.from_date as string)
              : undefined;
          const toDate =
            safeArgs.to_date !== undefined
              ? (safeArgs.to_date as string)
              : undefined;
          const format = safeArgs.format as "json" | "csv";

          const entries = _auditLog.export(fromDate, toDate);

          if (format === "csv") {
            const header =
              "timestamp,trace_id,tool_name,user_id,token_count,cost_usd";
