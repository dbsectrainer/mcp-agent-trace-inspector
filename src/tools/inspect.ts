 ...
 */
function detectReasoningChain(toolNames: string[]): {
  detected: boolean;
  patterns: string[];
} {
  ...
  const ACTION_PATTERNS =
    /action|execute|run|call|invoke|output|respond|answer/i;
  ...
  const standaloneReasoning = toolNames.filter((t) =>
  ...
  return {
    ...
    patterns:
      patterns.length > 0 ? patterns : standaloneReasoning.map((t) => t),
  };
}

export function handleGetTraceSummary(
  db: DatabaseSync,
  args: GetTraceSummaryArgs,
  ...
  try {
    ...
    throw new McpError(
      ...
      `Failed to get trace summary: ${message}`,
    );
  }
}

export function handleListTraces(
  db: DatabaseSync,
  args: ListTracesArgs,
): { content: Array<{ type: string; text: string }> } {
  const { limit } = args;
