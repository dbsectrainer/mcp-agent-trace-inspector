...
export function handleTraceStart(
  db: DatabaseSync,
  args: TraceStartArgs,
...
export function handleTraceStep(
  db: DatabaseSync,
  args: TraceStepArgs,
  ...
  try {
    ...
    let resolvedTokenCount: number | null = null;
    if (!noTokenCount) {
    ...
    console.error(
      `[trace_step] Recorded step "${tool_name}" for trace ${trace_id}, step id=${stepId}`,
    );
    return {
      content: [
        {
    ...
    throw new McpError(
      ErrorCode.InternalError,
      `Failed to record step: ${message}`,
    );
  }
}

export function handleTraceEnd(
  db: DatabaseSync,
  args: TraceEndArgs,
): { content: Array<{ type: string; text: string }> } {
  const { trace_id } = args;
  ...
  try {
    ...
    console.error(
      `[trace_end] Ended trace ${trace_id} (duration ~${durationMs}ms)`,
    );
    return {
      content: [
        {
